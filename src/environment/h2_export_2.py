...
class H2ExportEnvironmentTwoArg(DeterministicNode):
    ...
    X_SIZE = (
        5  # [lower_ramp_limit, upper_ramp_limit, lower_h2_storage, upper_h2_storage, energy_balance]
    )
    """
    ...
    # ---- Interface with Mu.F ---- #
    def __call__(
        self, u: jnp.ndarray, v: jnp.ndarray, z: Optional[jnp.ndarray] = None
    ) -> jnp.ndarray:
        assert (
            u.shape[-1] == self.U_SIZE
        ), f"Expected last dimension {self.U_SIZE}, got {u.shape[-1]}"
        assert (
            v.shape[-1] == self.V_SIZE
        ), f"Expected last dimension {self.V_SIZE}, got {v.shape[-1]}"

        if z is not None:
            assert z.shape[-1] == 1, f"Expected last dimension 1, got {z.shape[-1]}"
        else:
            z = 5.9 * jnp.ones_like(u[..., :1])

        return self.simulate(u, v, z)

    @partial(jax.jit, static_argnums=0)
    def simulate(
        self, u: jnp.ndarray, v: jnp.ndarray, z: jnp.ndarray = None
    ) -> jnp.ndarray:
        """
        ...
        energy_electrolysis = energy_electrolysis_eq(
            hydrogen_throughput, self._electrolyser_efficiency
        )

        energy_fuelcell = energy_fuelcell_eq(
            hydrogen_throughput, self._fuelcell_efficiency
        )
        ...
        upper_h2_storage_cons = hydrogen_storage_upper_cons(
            hydrogen_storage, self._upper_storage_limit
        )
        ...
        assert (
            combined.shape[-1] == self.Y_SIZE + self.X_SIZE + 1
        ), f"Expected last dimension {self.Y_SIZE + self.X_SIZE + 1}, got {combined.shape[-1]}"
...
def energy_balance_upper_cons(
    vector_energy, energy_electrolysis, energy_fuelcell, n_turbines, renewable_energy
):
