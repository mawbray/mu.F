...
class H2ExportEnvironmentTwoArg(DeterministicNode):
    ...
    # ---- Interface with Mu.F ---- #
    def __call__(self, u: jnp.ndarray, v: jnp.ndarray, z = jnp.ndarray) -> jnp.ndarray:
    ...
    def simulate(self, u: jnp.ndarray, v: jnp.ndarray, z: jnp.ndarray = None) -> jnp.ndarray:
        ...
        hydrogen_storage = jnp.clip(hydrogen_storage, a_min=self._upper_storage_limit *self._lower_storage_limit , a_max=self._upper_storage_limit)
...
@jax.jit
def vector_throughput_sat_l(_vector_throughput, _active_trains, vector_calorific_value, lower_ramp_limit, train_throughput_capacity): 
    return _vector_throughput - lower_ramp_limit * vector_calorific_value * _active_trains * train_throughput_capacity
...
@jax.jit
def vector_throughput_sat_u(_vector_throughput, _active_trains, vector_calorific_value, upper_ramp_limit, train_throughput_capacity, total_trains): 
    return _vector_throughput + vector_calorific_value * upper_ramp_limit * (total_trains - _active_trains + 1) * train_throughput_capacity
