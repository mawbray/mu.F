...
class H2ExportEnvironment(DeterministicNode):
    ...
    def simulate(self, u: jnp.ndarray, v: jnp.ndarray, z: jnp.ndarray = None) -> jnp.ndarray:
        ...
        energy_fuelcell = energy_fuelcell_eq(_renewable_energy, vector_energy)
...
@jax.jit
def hydrogen_fuelcell_eq(renewable_energy, vector_energy):
    """ Calculate the energy used by the fuelcell"""
    # GJ / h - GJ / h = GJ / h
    return jnp.maximum(vector_energy - renewable_energy, 0)
...
@jax.jit
def energy_fuelcell_eq(renewable_energy,  vector_energy):
    """ Calculate the energy used by the fuelcell """
    # GJ / h - GJ / h = GJ / h
    return jnp.maximum(vector_energy - renewable_energy, 0)
